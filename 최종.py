...
#help 화면
    ...
    while lf_run:
        ...
        while running:
            ...
            for i in range(int(elapsed_time/7)+1):

                # out of index 방지
                if i >= 5:
                    continue
                    print("컨티뉴 실행")
                # 각 장애물들의 위치 데이터를 리스트에 저장
            ...
            # 배경 빨간 테두리
            if int(elapsed_time/4) >= 4:
                bg_idx = 4
            else:
                bg_index = int(elapsed_time/4)
            ...
            timer = game_font.render(
                "버틴시간: "+str(int(elapsed_time)), True, (0, 0, 0))

            stage = game_font.render(
                "STAGE "+str(int(elapsed_time/5)+1), True, (0, 0, 0))

            # STAGE 4가 되면 글씨를 빨간색으로 바꾸고 STAGE 4에서 더 이상 올라가지 않음
            if int(elapsed_time) >= 20:
                stage = game_font.render("STAGE "+str(4), True, (255, 0, 0))
    ...
    while ud_run:
        ...
        character_width = character_size[0]
        character_height = character_size[1]
        character_x_pos = ((background_width)-(background_width / 8)) - (character_width / 2)
        character_y_pos = background_height - character_height
        character_speed = 0.4
        to_go = 0
        bg_index = 0
        ...
        while running:
            ...
            # 캐릭터의 y 포지션은 to_go의 값만큼 상시 이동
            character_y_pos += to_go * dt
            if character_y_pos >= screen_width - character_width:
            ...
            for i in range(int(elapsed_time/7)+1):
                ...
                if i >= 5:
                    continue
                    print("컨티뉴 실행")
                # 각 장매물들의 위치 데이터를 리스트에 저장
                ddong_x_pos[i] += (ddong_speed[i] * dt) + (elapsed_time / 10)
                if ddong_x_pos[i] > screen_height - ddong_height:
                    if dd==1:
                        if k[i] == 0:
                            k[i] = 1
                            ddong[i] = ddong4
                ...
                if ddong_x_pos[i] >= screen_height:
                    ddong_y_pos[i] = random.randrange(screen_width - ddong_width)
                    ddong_x_pos[i] = 0 - ddong_height
                    ddong[i] = ddong4
                    k[i] = 0
            ...
            # STAGE 5가 되면 글씨를 빨간색으로 바꾸고 STAGE 4에서 더 이상 올라가지 않음
            if int(elapsed_time) >= 20:
                stage = game_font.render("STAGE "+str(4), True, (255, 0, 0))
...
###########################################################            
    ...
    while run_ending:
        ...
        if character_x_pos >= 250:
            character_x_pos-=0.09
        elif ((int(elapsed_time)+1) % 2 ==0) and bk==0:
            bk=1
        elif ((int(elapsed_time)+1) % 2 ==1):
            bk=0
